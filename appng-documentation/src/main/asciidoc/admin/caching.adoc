...
----
* `cacheTimeouts` +
The path specific cache timeouts, as a multilined value. The format is `path-prefix = TTL in seconds`. Optionally, with appending `,no-cache`, you  can force a browser to not cache the resource locally. +
