...
<2> Next, define the content of the form using `<appNG:formData>`. The `mode` controls whether the form is always shown (`always`) or only as long it hasn't been submitted (`not_submitted`).
