...
[[app-messageSource]]
* `messageSource` +
This mesage source is built from the resource bundle file(s) provided in the application's `dictionary` folder. +
...
If your custom XSL stylesheets need to include custom resources from `application-home/resources`, the schema to build the path for this resources is:

`/template_<applicaton-name>/<relative-resource-path>`
