...
[TIP]
====
A method annotated with `@ExceptionHandler` can use a {appng}/org/appng/api/model/Site.html[Site^], {appng}/org/appng/api/model/Application.html[Application^] and {appng}/org/appng/api/Environment.html[Environment^] as a parameter.
====
