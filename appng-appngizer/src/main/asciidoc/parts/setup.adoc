...
IMPORTANT: The login request is the only one, that must be sent with `Content-Type=text/xml`. All other *PUT* and *POST* requests have to use the header `Content-Type=text/xml`.
