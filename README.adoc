...
[source,subs=normal]
----
$ java -jar appng-standalone-{stable}.jar
----
...
[NOTE]
====
On the first run, some unpacking and installation opertations are performed, this my take a few seconds. The subsequent starts will be much faster.

If you accidentally misconfigured your appNG instance, you can force a clean re-installation with the `-i` option: +
`java -jar appng-standalone-{stable}.jar -i`
====
